...
for gsIdx, gs_name in enumerate(GroundStates):
    ...
    TInit, TFinal = options["EMC"]["Temp"], 1e7
    dT = 0.02 * 1e88
    ...
    for i_tmp in range(iRange):
        for j_tmp in range(jRange):
            ...
            mu = mus[currMui]
            print("T = ", temp, "mu = ", mu)
            mcSGC = SGCMonteCarlo(gs05, temp, symbols=['Li', 'Na'])
            # Using move observer
            ...
            ################# ！！！！！！！！！！！！！！！！ ###################
            eq = equilibrium_detector(granularity=5, nb_bins=16, prec=options["EMC"]["error"], patience=6)
    ...
    np.save("lina-XTable-" + gs_name + ".npy", XTable)
    # np.save("lina-muTable-" + gs_db_name[-10:-7] + ".npy", muTable)
    np.save("lina-phiTable-" + gs_name + ".npy", phiTable)

    plt.plot(mus, phiTable[0], '-o')
