...
class KianCalc(Calculator):
    ...
    def __init__(self, energyFile, formationEBasis = None, endPointE = None, byOrder = False, relaxCalc = None):
            ...
            for idx, row in enumerate(tmpDb.select("")):
                if idx+1 not in toExclude:
                    ...
                    if not relaxCalc:
                        ...
                        atoms.calc = relaxCalc
                        ucf = UnitCellFilter(atoms)
                        opt = BFGS(ucf)
                        opt.run(fmax=0.02)
                        currE = atoms.get_potential_energy()
                    tmpDictE[atoms.symbols.formula._formula] = currE
                    ...
                    if formationEBasis is not None:
                        if endPointE is not None:
                            ...
                            if endPointTester == formationEBasis[0]:
                                energyEndPoints[0] = currE/len(row.numbers)
                            elif endPointTester == formationEBasis[1]:
                                energyEndPoints[1] = currE/len(row.numbers)
    ...
    def get_num(self, input_string, A, B):
        """
        return number of 'A' and 'B' in the input_string like BA2B4AB3A5B
        """
