...
class LTE:
    def __init__(self, myCalc = None, formation = False, species):
        ...
        self.species0 = species[0]
        self.species1 = species[1]
    ...
    def set_gs_atom(self, atom):
        ...
        self.gs_atom = atom
        self.N = len(atom)
        self.x = np.sum(self.gs_atom.numbers == self.species0)/self.N
        self.gsE0 = atom.info["gsE"][0]
        self.gsE1 = atom.info["gsE"][1]
        NSpecies0 = np.sum(self.gs_atom.numbers == self.species0)
        NSpecies1 = len(self.gs_atom) - NSpecies0
        ...
        if self.formation:
            ...
            self.gs_E = (atom.get_potential_energy() - NSpecies0 * self.gsE0 - NSpecies1 * self.gsE1)/len(atom)
    ...
    def get_E(self, T, mu):
        ...
        for i in range(self.N):
            ...
            if currSpecies == self.species0: 
                self.gs_atom.numbers[i] = self.species1
                dEta = -1
            if currSpecies == self.species1: 
                self.gs_atom.numbers[i] = self.species0
                dEta = 1
            ...
            NSpecies0 = np.sum(self.gs_atom.numbers == self.species0)
            NSpecies1 = len(self.gs_atom.numbers) - NSpecies0
            if self.formation:
                newE = self.gs_atom.get_potential_energy()
            else:
                newE = self.gs_atom.get_potential_energy() - NSpecies0 * self.gsE0 - NSpecies1 * self.gsE1
            # tmp = self.gs_atom.copy()
            # tmp.calc = self.myCalc
            # ucf = UnitCellFilter(tmp)
            # opt = FIRE(ucf, logfile=None)
