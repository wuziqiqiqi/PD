...
class TestStructureMapper(unittest.TestCase):
    def test_rattled_structures(self):
        tests = [
            {
        ...
        for test in tests:
            ...
            self.assertTrue(np.allclose(pos, pos_expect))

    def test_vacancies_no_distortion(self):
        ...
        self.assertAlmostEqual(x_to_au, 2.0/25.0)

        at_index, dist = neighbor_list('id', recovered, 3.0)
        coordination = np.bincount(at_index)
        self.assertTrue(np.all(coordination == 12))
        self.assertTrue(np.allclose(dist, 3.9/np.sqrt(2.0)))

    def test_vacancies_with_distortion(self):
        tests = [
            {
        ...
        mapper = StructureMapper()
        for test in tests:
            recovered, _ = mapper.snap_to_lattice(
                test['structure'], test['template']
            )
            for i in test['expect_vac']:
                self.assertEqual(recovered[i].symbol, 'X')
...
def mg2sn16x6_initial():
    numbers = [50, 50, 12, 50, 12, 50, 50, 50, 0, 50, 0, 50, 50, 50, 0, 50, 0,
               50, 50, 50, 0, 50, 0, 50]
...
def mg2sn16x6_final():
    numbers = [50, 50, 12, 50, 12, 50, 50, 50, 50, 50,
...
def mg5sn14x5_initial():
    numbers = [50, 50, 12, 50, 50, 12, 50, 50, 0, 12, 12, 50, 50,
               50, 0, 50, 0, 50, 50, 50, 0, 12, 0, 50]
    positions = [
        [0.0, 0.0, 0.0],
        [0.0, 3.375, 3.375],
        [1.6875, 1.6875, 1.6875],
...
def mg5sn14x5_final():
    numbers = [50, 50, 12, 50, 50, 12, 50, 50, 12, 12, 50,
               50, 50, 50, 50, 50, 50, 12, 50]
    ...
    return Atoms(numbers=numbers, cell=cell, positions=positions,
                 pbc=[1, 1, 1])
