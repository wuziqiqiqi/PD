...
class TestBayesianCompressiveSensing(unittest.TestCase):
    bayes = BayesianCompressiveSensing(fname=fname, output_rate_sec=2, maxiter=100)
    ...
    def test_fit_more_coeff(self):
        self.bayes = BayesianCompressiveSensing(fname=fname, noise=0.1, maxiter=1000)
        X = np.random.rand(30, 400)
    ...
    def test_fit_linear_dep_col(self):
        bayes = BayesianCompressiveSensing(fname=fname, noise=0.2, penalty=1E-2, maxiter=1000)
        X = np.random.rand(30, 400)
        X[:, 2] = X[:, 0]
        X[:, 8] = X[:, 20]
        X[:, 23] = X[:, 50]
