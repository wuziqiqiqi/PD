...
from clease.regression import LinearRegression, Lasso, Tikhonov, ConstrainedRidge
    ...
    def test_get_instance_array(self):
        ...
        for test in Test_dict:
            if test['scale'] == 'log':
                True_alpha.append(
                    np.ndarray.tolist(
                        np.logspace(np.log10(test['alpha_min']),
                ...
                True_alpha.append(
                    np.ndarray.tolist(
                        np.linspace(test['alpha_min'],
                                    test['alpha_max'],
                                    int(test['num_alpha']),
                                    endpoint=True)))

            Tik_list = Tikreg.get_instance_array(test['alpha_min'], test['alpha_max'],
                                                 test['num_alpha'], test['scale'])
...
class Test_Lasso(unittest.TestCase):
    ...
    def test_fit(self):
        ...
        sk_lasso = sklearn.linear_model.Lasso(alpha=1,
                                              fit_intercept=False,
                                              copy_X=True,
                                              normalize=True,
                                              max_iter=1e6)
    ...
    def test_get_instance_array(self):
        ...
        for test in Test_dict:
            ...
            las_list = lasreg.get_instance_array(test['alpha_min'], test['alpha_max'],
                                                 test['num_alpha'], test['scale'])
