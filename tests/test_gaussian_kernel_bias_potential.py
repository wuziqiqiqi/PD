...
class TestGaussianKernelBiasPotential(unittest.TestCase):
    def test_in_range(self):
        pot = GaussianKernelBiasPotential(xmin=0.0, xmax=1.0, num_kernels=100,
                                          width=0.05)
        ...
        mask[mask >= lower] = 0
        mask[mask <= upper] = 0
        self.assertTrue(np.all(kernels[mask] < 0.01))
    ...
    def test_evaluate(self):
        ...
        pot = GaussianKernelBiasPotential(
             xmin=0.0, xmax=1.0, num_kernels=10, width=0.01,
             getter=get_func)
    ...
    def test_get_index(self):
        pot = GaussianKernelBiasPotential(xmin=0.0, xmax=1.0, num_kernels=11,
                                          width=0.05)
        index = pot.get_index(0.5)
        self.assertEqual(index, 5 + pot.pad)
    ...
    def test_to_from_dict(self):
        ...
        num_kernels = 10
        width = 0.4
        pot = GaussianKernelBiasPotential(xmin=xmin, xmax=xmax,
        ...
        self.assertAlmostEqual(xmin, pot.xmin)
        self.assertAlmostEqual(xmax, pot.xmax)
        self.assertAlmostEqual(xmin_corr, pot.xmin_corrected)
        self.assertAlmostEqual(xmax_corr, pot.xmax_corrected)
        self.assertEqual(num_kernels, pot.num_kernels)
        self.assertAlmostEqual(width, pot.width)
        self.assertTrue(np.allclose(pot.centers, centers))
        self.assertTrue(np.allclose(pot.coeff, coeff))
    ...
    def test_ensure_zero_slope(self):
        pot = GaussianKernelBiasPotential()
        pot.local_update(0.01, 2)
        self.assertGreater(pot.slope(0.0), 0.0)
        pot.ensure_zero_slope(0.0)
        self.assertAlmostEqual(pot.slope(0.0), 0.0)
