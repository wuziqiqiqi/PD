import sys
sys.path.append("/u/wuziqi/PD/clease")
...
if args.device == "cpu":
    ...
    raise ValueError(f"Invalid device: {args.device}. Please choose from 'cpu' and 'gpu'")
...
for gsIdx in gsIdxs:
            ...
            if DEBUG:
                if len(mus) == 1:
                    ...
                    if batchMode:
                        ...
                        f = open(f"{inputFileName[:-5]}-reuslt/fmc-mu-noBatch.out", "a")
                    if iMu + iT == 0:
                        f.write(f"Going through gs: {gsIdxs}, T: {Ts}, mu: {mus}\n")
                    ...
                    if batchMode:
                        ...
                        f = open(f"{inputFileName[:-5]}-reuslt/{gs_name}-fmc-T-noBatch.out", "a")
                    if iMu + iT == 0:
                        f.write(f"Going through gs: {gsIdxs}, T: {Ts}, mu: {mus}\n")
    ...
    if len(Ts) == 1:      
        ...
        if batchMode:
            ...
            np.save(f"{inputFileName[:-5]}-reuslt/{gs_name}-{Ts[0]}-phiTable-noBatch.npy", phiTable[-1])
        plt.title(gs_name)
        # plt.show()
        plt.figure(2)
        ...
        if batchMode:
            ...
            np.save(f"{inputFileName[:-5]}-reuslt/{gs_name}-{Ts[0]}-XTable-noBatch.npy", XTable[-1])
        plt.title(gs_name)
        ...
        if batchMode:
            ...
            for idx, ts in enumerate(Ts):
                startingPoints[str(ts)] = phiTable[idx,0]
            with open(f"{inputFileName[:-5]}-reuslt/{gs_name}-startingPoints-noBatch.json", 'w') as f:
                json.dump(startingPoints, f)
...
with open(f"{inputFileName[:-5]}-reuslt/walltime.txt", 'w') as f:
    f.write(f"Batch: {args.batch}, Init: {args.init}, Temp: {args.temp}, gs: {args.gs} walltime: {time.time() - startt}")
