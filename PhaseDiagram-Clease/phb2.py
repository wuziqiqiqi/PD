...
ASElammpsTmp = LAMMPSlib(lmpcmds=cmds, keep_alive=True, log_file=os.path.join(rootDir, "LAMMPSloggsTmp.log"))
# lmp = lammps()
...
db_name = "LiNa/LiNa-merged-new.db"
...
with open('LiNa/LiNa-merged-new.dbwtf4-eci.json') as f:
    eci = json.load(f)
...
"""
gs0_db_name = "LiNa/LiNa-64-1.0-EIM.db"
...
gs1_db_name = "LiNa/LiNa-64-0.0-EIM.db"
...
db = connect("LiNa/LiNa-64-1.0-EIM.db")
gsPrep = None
for row in db.select(""):
    gsPrep = row.toatoms()
...
with open("phbLog-lina-EIM-acc0.003.txt", "r") as f:
    lines = f.readlines()
    for line in lines:
        tmp = line.split("\t")
        lroPhase[0].append(float(tmp[0]))
        lroPhase[1].append(float(tmp[1]))
...
while 1:
    ...
    while 1:
        ...
        while 1:
            newMu += dMu
            gsTmp = gsPrep.copy()
            gsTmp.calc = ASElammpsTmp
            systemSize = len(gsTmp.numbers)
            mcSGC = SGCMonteCarlo(gsTmp, temp, symbols=['Li', 'Na'])
            numOfAu = []
            MaxIterNum = 1000000
            eq = equilibrium_detector(granularity=5, nb_bins=16, prec=phbPrec, patience=6)
            for i in range(MaxIterNum):
                ...
                if eq.new_data(POI=currX, other=0) or i == MaxIterNum-1:
                    ...
                    if 1:
                        # lroPhase[phase][-1] = np.sum(old_spins != gss[phase].numbers)
                        tmpPOI[phase] = eq.get_POI()
                        old_spins = np.copy(gsTmp.numbers)

                    # reset chemical potential
                    tmpNum = gsTmp.numbers
                    mcSGC.run(steps=0, chem_pot={'c1_0': 0})
                    for (a, b) in zip(tmpNum, gsTmp.numbers):
                        assert a == b
                    # exit()
                    break
            if tmpPOI[phase] < 0.5 and len(lroTmp) == 3:
                newMu -= 0.6
                continue
            if disconti_detector(ys=lroTmp,newY=tmpPOI[phase], mu = newMu) > Z:
                mu = newMu - dMu
                break
            lroTmp.append(tmpPOI[phase])
        oldDT = np.inf
        oldDMu = np.inf
    ...
    logF = open("phbLog-lina-EIM-acc0.003.txt", 'a')
