...
for idx, xxx in enumerate(xs):
    ...
    # tmpAtoms = SO.relax(atoms=AseAtomsAdaptor.get_structure(tmpAtoms), fmax=0.2, relax_cell=True, save_path=os.path.join(os.getcwd(), str(xxx)), verbose=True)["final_structure"]
    # tmpAtoms = AseAtomsAdaptor.get_atoms(tmpAtoms)
    
    # cell relax
    chgnet = CHGNet.load()
    relaxScale  = np.linspace(1, 1.05, 50, endpoint=True)
    Emin = np.inf
    rsmin = 1
    chgnetCalc = CHGNetCalculator(model=chgnet)
    for rs in relaxScale:
        tmptmpAtoms = tmpAtoms.copy()
        tmptmpAtoms.set_cell(tmpAtoms.get_cell()*rs, scale_atoms=True)
        tmptmpAtoms.calc = chgnetCalc
        E1 = tmptmpAtoms.get_potential_energy()
        if E1 < Emin:
            Emin = E1
            rsmin = rs
    print(f"Emin = {Emin}, rsmin = {rsmin}")
    tmpAtoms.set_cell(tmpAtoms.get_cell()*rsmin, scale_atoms=True)
            
    write_lammps_data(file=subDir + "/initial-solid.data", atoms=tmpAtoms, atom_style="full", masses=True)
...
velocity       all create 1 1312343 dist gaussian
...
fix             1 all npt temp 1 {initialTemp} $(100.0*dt) aniso 1.01325 1.01325 $(1000.0*dt)
run             10000
unfix           1

fix             2 all npt temp {initialTemp} {initialTemp} $(100.0*dt) aniso 1.01325 1.01325 $(1000.0*dt)
run             10000
unfix           2

fix             3 gLiquid npt temp {initialTemp} 1500 $(100.0*dt) z 1.01325 1.01325 $(1000.0*dt) couple none
run             20000
unfix           3
