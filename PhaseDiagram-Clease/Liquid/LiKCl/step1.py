...
for myDir in myDirs:
    ...
    structureB = read_lammps_data(myDir + "/structureA.data")
    for idx, i in enumerate(lmpAtomConversion):
    ...
    for T in Ts:
        ...
        with open(subDir + "/mixed.in", 'w') as f:
            ...
            f.write("thermo\t10\n")
            f.write("dump\t1 all custom 10 xyz-mix.dump type x y z\n")
            f.write("dump\t2 all custom 10 force-mix.dump type fx fy fz\n")
            f.write(
                """
fix     1 all nve
run     2000
...
mpirun -np $SLURM_NTASKS /jet/home/wuziqi/clease/lammps-stable_29Sep2021_update3/GAPbuild/lmp -in mixed.in > mixed.log
