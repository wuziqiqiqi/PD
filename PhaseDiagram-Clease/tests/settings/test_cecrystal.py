...
def test_two_grouped_basis(db_name, check_cf):
    ...
    basis = [(0.00, 0.00, 0.00), (1.0 / 3, 2.0 / 3, 0.00), (1.0 / 3, 0.00, 0.25)]
    bsg = CECrystal(
        concentration=conc,
        spacegroup=167,
        basis=basis,
...
def test_unequal_number_of_sites(make_conc, db_name):
    ...
    conc = make_conc(
        basis_elements=[["Na", "Cl"], ["Fe"], ["O"], ["O"]],
        grouped_basis=[(0,), (1,), (2, 3)],
    )

    conc.set_conc_formula_unit(
        formulas=["Na<x>Cl<6-x>", "Fe<3>", "O<6>"], variable_range={"x": (0, 6)}
    )
    settings = CECrystal(
        concentration=conc,
        spacegroup=187,
