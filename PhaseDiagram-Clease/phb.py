...
db_name = options["CLEASE"]["CESettings"]["db_name"]
conc = Concentration(basis_elements=options["CLEASE"]["CESettings"]["concentration"])

tmp = basis_elements=options["CLEASE"]["CESettings"].copy()
tmp['concentration'] = conc
MCsettings = CEBulk(**tmp)

with open(options["CLEASE"]["CEFitting"]["ECI_filename"]) as f:
    eci = json.load(f)
...
"""
gs0_db_name = options["PHB"]["gs0_db_name"]
...
# gs0.calc = ASElammps0
gs0 = attach_calculator(MCsettings, atoms=gs0, eci=eci)
# gs05.calc = EAM(potential='AuCu_Zhou04.eam.alloy')

view(gs0)
print("gs atom E:", gs0.get_potential_energy())

gs1_db_name = options["PHB"]["gs1_db_name"]
...
# gs1.calc = ASElammps1
gs1 = attach_calculator(MCsettings, atoms=gs1, eci=eci)
...
TInit = options["PHB"]["TInit"]
muInit = options["PHB"]["muInit"]
dT = options["PHB"]["dT"]
dMuMax = options["PHB"]["dMu"]
oldDT = np.inf
oldDMu = np.inf
...
phbPrec = options["PHB"]["error"]

if os.path.exists("phbLog-lina-CLEASE-acc"+str(phbPrec)+"-fine-64.txt"):
    os.remove("phbLog-lina-CLEASE-acc"+str(phbPrec)+"-fine-64.txt")

while 1:
    tmpGs = [gss[0].copy(), gss[1].copy()]
    tmpPOI = np.array([np.inf,np.inf])
    if temp > options["PHB"]["TFinal"]:
        print("ha!")
        break
    secondpass = 0
    while 1:
        ...
        print(discontPhase[0], "\t", discontPhase[1])
        ...
        while 1:
            newMu += dMu
            systemSize = len(gss[phase].numbers)
            mcSGC = SGCMonteCarlo(gss[phase], temp, symbols=['Li', 'Na'])
            numOfAu = []
            MaxIterNum = 1000000
            eq = equilibrium_detector(granularity=5, nb_bins=16, prec=phbPrec, patience=6)
            for i in range(MaxIterNum):
                if not i % 100000:
                    print(i)
                mcSGC.run(steps=100, chem_pot={'c1_0': newMu})
                currX = get_conc(gss[phase])
                numOfAu.append(currX)
                # if converged, save qautities of interest, get ready to integrate
                if eq.new_data(POI=currX, other=0) or i == MaxIterNum-1:
                    ...
                    # exit()
                    break
            if (disconti_detector(ys=lroPhase[phase][:-1],newY=tmpPOI[phase], mu = newMu) < disconti_detector(ys=lroPhase[1-phase][:-1],newY=tmpPOI[phase], mu = newMu)):
                break
        muBound[0] = newMu
        dMu = -dMu
        while 1:
            ...
            for i in range(MaxIterNum):
                ...
                if eq.new_data(POI=currX, other=0) or i == MaxIterNum-1:
                    ...
                    # exit()
                    break
            if (disconti_detector(ys=lroPhase[1-phase][:-1],newY=tmpPOI[phase], mu = newMu) < disconti_detector(ys=lroPhase[phase][:-1],newY=tmpPOI[phase], mu = newMu)):
                break
        muBound[1] = newMu
    ...
    logF = open("phbLog-lina-CLEASE-acc"+str(phbPrec)+"-fine-64.txt", 'a')
